        ...
        match self.segment.ctrls {
            ...
            _ => {
                while self.index < self.segment.value_lut.len() {
                    if offset == self.segment.value_lut[self.index] { 
                        return self.segment.point_lut[self.index];
                    } else if offset < self.segment.value_lut[self.index] {
                        break;
                    }
                    self.index += 1;
                }
